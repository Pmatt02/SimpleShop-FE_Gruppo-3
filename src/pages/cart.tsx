  ...
  const emptyOrNot=(products:CartItem[], option:"card" | "table")=>{
    if(products.length){
      if(option==="card"){
        ...
        return (<div className="w-full sm:max-w-sm sm:mx-auto  sticky top-[20px]"><Resoconto products={cart} /></div>)
      }
    }else{
      if(option==="card"){
        return( <div className="w-full flex justify-center-safe">
          <div className="flex-wrap p-20">
            <h2 className="font-extrabold text-3xl text-red-900">Il carrello è vuoto</h2>
            <p>Vai alla <Link to={"/"} className="underline decoration-sky-500">Home</Link> e vedi i nostri prodotti</p>
          </div>
     </div>)
      }
